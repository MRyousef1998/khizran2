...
class ProductesController extends Controller
{
    public function get_productes_with_category(Request $request){
       ...
       // return $request;
        ...
        ->leftJoin('statuses', 'products.statuses_id', '=', 'statuses.id') ->Join('order_product', 'products.id', '=', 'order_product.products_id')->where("product_details.category_id",$request->category_id)->where("product_details.rate",'>=',4)->where("products.selling_date", null)
        ...
        $allData['machines_of_this_category']=ProductItemRecource::collection( $machines_of_this_category) ;
